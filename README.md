...
**Notification daemon for Raspberry Pi Zero W** — listens for notifications via MQTT, then drives an OLED display, NeoPixel LEDs, a piezo buzzer, and a ToF proximity sensor for wave-to-dismiss.

A Mochisuki device sits on your desk and acts as a physical notification terminal. Any Hermes-compatible agent (or your own scripts) can publish a notification to `hermes/notify`, and the daemon handles the rest: visual display, urgency escalation, wave-to-dismiss, and persistent event logging.
...
| `proximity.py` / `display.py` / `leds.py` / `buzzer.py` | Async hardware drivers (stub gracefully) |
